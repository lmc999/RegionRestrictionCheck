...
while getopts ":I:M:ESX:T:P:" optname; do
  ...
  "T")
    silence="1"
    case "$OPTARG" in
    1 | 2 | 3 | 4 | 5 | 6 | 7 | 8 | 0 | 99)
      num="$OPTARG"
      ;;
    *)
      echo "Invalid value for key -T: $OPTARG."
...
Media_Cookie=$(curl -s --retry 3 --max-time 10 "https://raw.githubusercontent.com/bere333a/RegionRestrictionCheck/main/cookies")
IATACode=$(curl -s --retry 3 --max-time 10 "https://raw.githubusercontent.com/bere333a/RegionRestrictionCheck/main/reference/IATACode.txt")
IATACode2=$(curl -s --retry 3 --max-time 10 "https://raw.githubusercontent.com/bere333a/RegionRestrictionCheck/main/reference/IATACode2.txt" 2>&1)
...
# Streaming Unblock Test - Animation Insane
...
# Streaming Unblock Test - Bilibili Continent Exclusive
MediaUnlockTest_BilibiliChinaMainland() {
...
# Streaming unblock test - Bilibili Hong Kong, Macau and Taiwan only
...
# Streaming Unblock Test - Bilibili Taiwan Limited
...
# Streaming Unblock Test - Abema.TV
...
MediaUnlockTest_HuluUS() {
  if [[ "$1" == "4" ]]; then
    curl $useNIC $xForward -fsL -o ./Hulu4.sh.x https://github.com/bere333a/RegionRestrictionCheck/raw/main/binary/Hulu4${arch}.sh.x >/dev/null 2>&1
    chmod +x ./Hulu4.sh.x
    ./Hulu4.sh.x >/dev/null 2>&1
  elif [[ "$1" == "6" ]]; then
    curl $useNIC $xForward -fsL -o ./Hulu6.sh.x https://github.com/bere333a/RegionRestrictionCheck/raw/main/binary/Hulu6${arch}.sh.x >/dev/null 2>&1
    chmod +x ./Hulu6.sh.x
...
MediaUnlockTest_NetflixCDN() {
  ...
  #local IATACode2=$(curl -s --retry 3 --max-time 10 "https://raw.githubusercontent.com/bere333a/RegionRestrictionCheck/main/reference/IATACode2.txt" 2>&1)
...
MediaUnlockTest_ElevenSportsTW() {
...
MediaUnlockTest_GYAO() {
  ...
  if [ "$result" ] && [ "$isOutsideJapan" ]; then
    echo -n -e "\r GYAO!:\t\t\t\t\t${Font_Red}No${Font_Suffix}\n"
  elif [ "$result" ] && [ -z "$isOutsideJapan" ]; then
...
MediaUnlockTest_J:COM_ON_DEMAND() {
...
MediaUnlockTest_Instagram.Music() {
...
MediaUnlockTest_MeWatch() {
  local tmpresult=$(
    curl -s $useNIC $usePROXY $xForward -${1} ${ssll} --max-time 10 https://cdn.mewatch.sg/api/items/97098/videos?delivery=stream%2Cprogressive &
    ff=idp%2Cldp%2Crpt%2Ccd &
    lang=en &
    resolution=External &
    segments=all
  )
...
Goodbye() {
  ...
  if [[ -z "$silence" ]]; then
    if [[ "$language" == "e" ]]; then
      ...
      echo -e ""
      #bash <(curl -s https://raw.githubusercontent.com/bere333a/RegionRestrictionCheck/main/reference/AD/AD${ADN})
      bash <(curl -s https://raw.githubusercontent.com/bere333a/RegionRestrictionCheck/main/reference/AD/ADDV)
      echo -e ""
      bash <(curl -s https://raw.githubusercontent.com/bere333a/RegionRestrictionCheck/main/reference/AD/AD2)
    fi
  fi
}
...
ScriptTitle() {
  if [[ -z "$silence" ]]; then
    if [[ "$language" == "e" ]]; then
      ...
      echo ""
      echo -e "${Font_Green}Github Repository:${Font_Suffix} ${Font_Yellow} https://github.com/bere333a/RegionRestrictionCheck ${Font_Suffix}"
      ...
      echo ""
      echo -e "${Font_Green}项目地址${Font_Suffix} ${Font_Yellow}https://github.com/bere333a/RegionRestrictionCheck ${Font_Suffix}"
