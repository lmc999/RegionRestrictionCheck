...
function MediaUnlockTest_Netflix() {
	...
	if [[ "$result1" == "404" ]]; then
		...
		local region=$(curl $useNIC -${1} --user-agent "${UA_Browser}" -fs --max-time 10 --write-out %{redirect_url} --output /dev/null "https://www.netflix.com/title/80018499" | cut -d '/' -f4 | cut -d '-' -f1 | tr [:lower:] [:upper:])
		if [[ ! -n "$region" ]]; then
			region="US"
		fi
...
function MediaUnlockTest_TVer() {
	echo -n -e " TVer:\t\t\t\t\t->\c"
	local tmpresult=$(curl $useNIC --user-agent "${UA_Browser}" -${1} -Ss --max-time 10 -H "${TVer_Cookie}" "https://edge.api.brightcove.com/playback/v1/accounts/5102072605001/videos/ref%3Akaguyasama_01 " 2>&1)
	if [[ "$tmpresult" == "curl"* ]]; then
