...
function CheckV4() {
	echo -e " ${Font_SkyBlue}** Checking Status in IPv4${Font_Suffix} "
	echo "--------------------------------"
	echo -e " ${Font_SkyBlue}** Your Network: ${local_isp}${Font_Suffix} "
	check4=`ping 1.1.1.1 -c 1 2>&1`;
	if [[ "$check4" != *"unreachable"* ]] && [[ "$check4" != *"Unreachable"* ]];then
		isv4=1
	else
		echo -e "${Font_SkyBlue}Your VM does not support IPv4, skip to next stage${Font_Suffix}"
		isv4=0
	fi
...
if [[ "$check6" != *"unreachable"* ]] && [[ "$check6" != *"Unreachable"* ]];then
	echo ""
	echo ""
	echo -e " ${Font_SkyBlue}** Checking Status in IPv6${Font_Suffix} "
	echo "--------------------------------"
	echo -e " ${Font_SkyBlue}** Your Network: ${local_isp}${Font_Suffix} "
    isv6=1
else
    echo -e "${Font_SkyBlue}Your VM does not support IPv6, skip to next stage${Font_Suffix}"
	ipv6=0
fi
echo -e "";
}
...
function Goodbye(){
echo -e "${Font_Green}All tests are done! Thanks for using the script! ${Font_Suffix}";
}

clear;

function ScriptTitle(){
echo -e "OTT Platform Restriction Test";
echo ""
echo ""
echo -e " ** Testing Time: $(date)";
echo ""
}
ScriptTitle

function Start(){
echo -e "${Font_Blue}Please select test subjects，or press ENTER to run full region test${Font_Suffix}"
echo -e "${Font_SkyBlue}Input number【1】：【Global & US Platforms】${Font_Suffix}"
echo -e "${Font_SkyBlue}Input number【2】：【Global & JP Platforms】${Font_Suffix}"
echo -e "${Font_SkyBlue}Input number【3】：【Global & HK Platforms】${Font_Suffix}"
echo -e "${Font_SkyBlue}Input number【4】：【Global & TW Platforms】${Font_Suffix}"
echo -e "${Font_SkyBlue}Input number【5】：【Global & EU Platforms】${Font_Suffix}"
echo -e "${Font_SkyBlue}Input number【6】：【Global Platforms Only】${Font_Suffix}"
...
function RunScript(){
	if [[ -n "${num}" ]]; then
		if [[ "$num" -eq 1 ]]; then
			...
			echo -e "${Font_Red}Wrong number input! Please re-run the script with correct number!${Font_Suffix}"
			return
		fi
	else
		clear
